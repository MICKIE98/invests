...
! function(i) {
    i.fn.equalHeights = function(n) {
        var e, o = {
            ...
            d = 0,
            c = i(this);
        return c.length < 2 ? this : (t.onResize && i(window).resize(function() {
        ...
        }), t.onLoad && i(window).load(function() {
            c.equalHeights({
                onResize: !1,
                onLoad: !1
            })
        }), c.each(function() {
            u = i(this), u.height("auto"), a = u.height(), a > 0 && (d = u.position().top, s = d in h, s ? a > h[d] ? (h[d] = a, i(c).filter(function() {
                return i(this).position().top == d
            }).height(a)) : u.height(h[d]) : (h[d] = a, u.height(h[d])))
        }), this)
    }
}(jQuery);
/* Tabs */
...
(function() {
            ...
            if (option.nodeName.toUpperCase() === "OPTION") {
                if (option.text !== "") {
                    ...
                    this.parsed.push({
                        ...
                        value: option.value,
                        text: option.getAttribute('data-option-html') !== null ? option.getAttribute('data-option-html') : option.text,
                        html: option.getAttribute('data-option-html') !== null ? option.getAttribute('data-option-html') : option.innerHTML,
    ...
    Chosen = (function(_super) {
        ...
        Chosen.prototype.results_reset = function() {
            ...
            this.show_search_field_default();
            this.results_reset_cleanup();
            this.form_field_jq.trigger("chosen:results_reset", {
                chosen: this
            });
            if (this.active_field) {
                return this.results_hide();
...
function openformYouTube(elm) {
    $(elm).fadeIn(100);
    var video = $(elm).attr('data-video');
    $(elm + ' .video-area').html('<iframe src="' + video + '" frameborder="0" allowfullscreen></iframe>');
    $('body').css('overflow', 'hidden');
}


function reviewAdd() {
    var form = 'form[name="form-review"]';
    var postData = $(form).serialize() + '&key=' + getUniqueId();

    $(form + ' .loading').fadeIn(0);

    $.post("?page_load=ajax&url=/ajax/reviews.ajax", postData, function(data) {
        data = JSON.parse(data);

        /* success */
        if (data['status'] == 1) {
            $(form).html('<div class="infobox">' + data['message'] + '</div>');
        } else {
            smoke.alert(data['message']);
        }

    }).done(function() {
        $(form + ' .loading').fadeOut(1000);
    });
}

function reviewType(val) {
    val = (val == 1) ? 1 : 0;
    $('#review-type').val(val);
}
...
function createMessage(type) {
    ...
    var postData = $(form).serialize() + '&key=' + getUniqueId() + '&type=' + type;

    $(form + ' .loading').fadeIn(0);

    $.post("?page_load=ajax&url=/ajax/feedback.ajax", postData, function(data) {
        data = JSON.parse(data);
...
function signin() {
    return false; // todo need analyzing
    var form = 'form[name="form-login"]';
    var postData = $(form).serialize() + '&key=' + getUniqueId();

    $(form + ' .loading').fadeIn(0);

    $.post("?page_load=ajax&url=/ajax/signin.ajax", postData, function(data) {
        data = JSON.parse(data);

        /* success */
        if (data['status'] == 1) {
            $(form).html('<center>Redirecting...</center>');
            location.reload();
        } else {
            smoke.alert(data['message']);
        }
...
$(function() {
    ...
    $(".btn-vote").on("click", function() {
        ...
        $(".btn-vote").removeClass("is-active");
    ...
    $('#presentation').click(function() {
        $('#presentation .video-area').html('');
    });

    /* Tabs currency-attrs */
    $(".currency-attrs a").tabs();
    ...
    function refreshTime() {
        ...
        var formattedString = dateString.replace(",", " ").replaceAll('/', '.');
        timeDisplay.innerHTML = formattedString + ' UTC';
    }
...
/** Custom group plans for home */
function viewGroupPlans() {
    var group = $('#group_plans').val();
    var ulList = $('ul#list_group_plans');
    ...
    // Detect if exist in list active or no
    if (needReselect) {
        ulList.children('li:not(.hide):first').children('a').click();
    }
}
...
$(document).ready(function() {
    ...
    $('#group_plans').on('change', viewGroupPlans);
});
...
function popupSwitch(popup1, popup2) {

    $(popup1).fadeOut(0, function() {
        $(popup2).fadeIn(100);
    });
